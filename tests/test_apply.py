...
class TestApply:
    def test_apply(self, fake_output, fake_pdf):
        ...
        # create object
        o = output.Output(fake_output)
        # fake pdfs
        pdf_grid = apply.apply_pdf(o, fake_pdf)
        assert len(pdf_grid) == len(fake_output["Q2grid"])
        pdfs = pdf_grid[q2_out]["pdfs"]
        assert list(pdfs.keys()) == fake_output["targetpids"]
        ref_pid1 = fake_output["Q2grid"][q2_out]["operators"][0, :, 1, :] @ np.ones(
            len(fake_output["interpolation_xgrid"])
        )
        np.testing.assert_allclose(pdfs[0], ref_pid1)
        ref_pid2 = fake_output["Q2grid"][q2_out]["operators"][1, :, 1, :] @ np.ones(
            len(fake_output["interpolation_xgrid"])
        )
    ...
    def test_apply_flavor(self, fake_output, fake_pdf, monkeypatch):
        ...
        monkeypatch.setattr(
            "eko.basis_rotation.flavor_basis_pids", fake_output["targetpids"]
        )
        fake_evol_basis = ("a", "b")
        ...
        ref_a = (
            fake_output["Q2grid"][q2_out]["operators"][0, :, 1, :]
            + fake_output["Q2grid"][q2_out]["operators"][1, :, 1, :]
        ) @ np.ones(len(fake_output["interpolation_xgrid"]))
