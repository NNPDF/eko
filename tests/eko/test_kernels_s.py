    ...
    for method in methods:
        try:
            np.testing.assert_allclose(
                s.dispatcher(
                    (0, 0), method, gamma_s, 1, 1, nf, ev_op_iterations, ev_op_max_order
                ),
                np.zeros((2, 2)),
            )
            np.testing.assert_allclose(
...
def test_error():
    with pytest.raises(NotImplementedError):
        s.dispatcher((2, 0), "AAA", np.random.rand(3, 2, 2), 0.2, 0.1, 3, 10, 10)
...
def test_singlet_back():
    order = (2, 0)
    gamma_s = (
        np.random.rand(order[0] + 1, 2, 2) + np.random.rand(order[0] + 1, 2, 2) * 1j
    )
    nf = 4
    a1 = 3.0
    a0 = 4.0
