import numpy as np
import pytest
from ekore_rs import Cache, ome_us
...
def test_a_non_singlet():
    ...
    a = ome_us.A_non_singlet(3, c, nf, L)
    ...
    # N3LO
    assert_allclose(a[2], np.zeros((2, 2)), atol=1e-14)
...
def test_a_singlet():
    ...
    # N3LO
    assert_allclose(a[2], np.zeros((3, 3)), atol=1e-14)
...
def test_guards_invalid_order_raises():
    ...
    with pytest.raises(ValueError):
        ome_us.A_singlet(4, c, nf, L)
    with pytest.raises(ValueError):
        ome_us.A_non_singlet(4, c, nf, L)
