...
pub fn gamma_ps(c: &mut Cache, nf: u8, variation: u8) -> Complex<f64> {
    ...
    let y1L4cff = -1.3168724 * 10. * nf_;
    let y1L3cff = -1.9911111 * f64::pow(10., 2) * nf_ + 1.3695473 * 10. * nf2;

    // Known small-x coefficients
    let bfkl1 = 1.7492273 * f64::pow(10., 3) * nf_;
    let x0L6cff = -7.5061728 * nf_ + 7.9012346 * 0.1 * nf2;
    let x0L5cff = 2.8549794 * 10. * nf_ + 3.7925926 * nf2;
    let x0L4cff =
        -8.5480010 * f64::pow(10., 2) * nf_ + 7.7366255 * 10. * nf2 - 1.9753086 * 0.1 * nf3;
...
mod tests {
    ...
    #[test]
    // Test the prediction of N=22 wrt to [Falcioni:2023luc].
    ...
    fn test_reference_moments() {
        fn qq3ps_moment(N: usize, nf: f64) -> f64 {
            let nf2 = nf * nf;
            let nf3 = nf2 * nf;
            // From Eq. 14 of [Falcioni:2023luc].
            let mom_list = [
