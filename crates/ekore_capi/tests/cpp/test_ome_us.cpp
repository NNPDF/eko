...
static int test_a_non_singlet()
{
    ...
    /* LO */
    for (int k = 0; k < 4; k++)
        fail |= check("a_non_singlet", "LO", a[k], 0., 0., 1e-14);
    /* NNLO */
    for (int k = 4; k < 8; k++)
        fail |= check("a_non_singlet", "NNLO", a[k], 0., 0., 1e-14);
...
static int test_a_singlet()
{
    ...
    /* LO */
    ComplexF64 lo_col2 = {a[2].re + a[5].re + a[8].re, a[2].im + a[5].im + a[8].im};
    fail |= check("a_singlet", "LO col2 sum", lo_col2, 0., 0., 1e-10);
    ComplexF64 lo_col0 = {a[0].re + a[3].re + a[6].re, a[0].im + a[3].im + a[6].im};
    fail |= check("a_singlet", "LO col0 sum", lo_col0, 0., 0., 1e-12);

    /* NNLO */
    ComplexF64 nnlo_col0 = {a[9].re + a[12].re + a[15].re, a[9].im + a[12].im + a[15].im};
    fail |= check("a_singlet", "NNLO col0 sum", nnlo_col0, 0., 0., 2e-6);
    ComplexF64 nnlo_col1 = {a[10].re + a[13].re + a[16].re, a[10].im + a[13].im + a[16].im};
    fail |= check("a_singlet", "NNLO col1 sum", nnlo_col1, 0., 0., 1e-11);
...
static int test_guards()
{
    ...
    for (int i = 0; i < 9; i++)
        fail |= check("guards", "A_singlet order3 untouched", a_s[i], sentinel.re, sentinel.im, 0.);

    ComplexF64 a_ns[4] = {sentinel, sentinel, sentinel, sentinel};
    ome_us_A_non_singlet(3, c, nf, L, a_ns, 4);
    for (int i = 0; i < 4; i++)
        fail |= check("guards", "A_non_singlet order3 untouched", a_ns[i], sentinel.re, sentinel.im, 0.);
