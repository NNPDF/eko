...
def save_all_operators_to_pdf(path, ret):
    ...
    pp = PdfPages(path)
    # NS
    fig = plot_operator(ret, "NS", log_operator=False)
    pp.savefig()
    plt.close(fig)
    fig = plot_operator(ret, "NS", abs_operator=True)
    pp.savefig()
    plt.close(fig)
    # Singlet
    fig = plot_operator(ret, "S_qq")
    pp.savefig()
    plt.close(fig)
    fig = plot_operator(ret, "S_qg")
    pp.savefig()
    plt.close(fig)
    fig = plot_operator(ret, "S_gq")
    pp.savefig()
    plt.close(fig)
    fig = plot_operator(ret, "S_gg")
    pp.savefig()
    plt.close(fig)
    # close
    pp.close()
