...
def beam_function_from_pdf(
    eko: EKO, lhapdf_like, mu2, targetgrid=None, flavor_rotation=None, labels=None
):
    """
    Apply all available operators to the input PDFs.
    ...
    for j, pid in enumerate(eko.bases.inputpids):
        if not lhapdf_like.hasFlavor(pid):
            continue
        pdfs[j] = np.array(
            [lhapdf_like.xfxQ2(pid, x, mu2) / x for x in eko.bases.inputgrid.raw]
        )
