...
@nb.njit(cache=True)
def gamma_nsv(N, nf, cache, is_singlet=None):
...
@nb.njit(cache=True)
def gamma_qq(N, nf, cache, is_singlet=None):
    """Compute the NNLO quark-quark anomalous dimension.

    To be added post benchmark.
    ...
    S2 = w2.S2(N)  # c.get(c.S2, cache, N, is_singlet)
    S3 = w3.S3(N)  # c.get(c.S3, cache, N, is_singlet)
    S4 = w4.S4(N)  # c.get(c.S4, cache, N, is_singlet)
    S1M = S1 - NI
    S11 = S1 + N1I
    S12 = S11 + N2I
    S2M = S2 - NI2
...
def gamma_singlet(N, nf, cache, is_singlet=True):
    ...
    result = np.array(
        [
            [gamma_qq(N, nf, cache, is_singlet), gamma_gq(N, nf, cache, is_singlet)],
