...
def gamma_ps_nf1(n, sx, variation):
    ...
    if variation == 1:
        fit = 2199.5733178661903/np.power(-1. + n,2) + 50220.76106506934/np.power(n,4) - 25639.474370323638/np.power(n,3) - (30.464637396723777*np.power(S1,3))/np.power(n,2)
    elif variation == 2:
        ...
        fit = 10724.193950510604/np.power(-1. + n,2) - 91906.6874849254/np.power(n,4) - 18075.198538003904/(n + np.power(n,2)) + (166.20937161911985*np.power(S1,3))/np.power(n,2)
    elif variation == 5:
        ...
        fit = 18207.919823885208/np.power(-1. + n,2) - 198879.25331350215/np.power(n,4) - 35873.119396447386/np.power(1. + n,2) + (374.1246977286895*np.power(S1,3))/np.power(n,2)
    elif variation == 7:
        ...
        fit = 4497.207022132304/np.power(-1. + n,2) - 15656.013490529653/np.power(n,2) + (4295.2328201943355*S1)/np.power(n,2) - (102.14388946214294*np.power(S1,3))/np.power(n,2)
    elif variation == 11:
        ...
        fit = 7262.768247576791/np.power(-1. + n,2) - 1263.1828974226962/(-1. + n) - 35781.181966367854/np.power(n,4) - 2647.104829494278/np.power(n,3) - 1578.388706824267/np.power(n,2) + 1263.1828974226962/n - 5751.779093981027/np.power(1. + n,3) + 5316.999890430546/np.power(1. + n,2) - 1732.8614496090886/(n*(2. + n)) - 3678.344227797872/(n + np.power(n,2)) - (4352.840748137642*S1)/np.power(n,2) + (399.4050307707205*np.power(S1,2))/np.power(n,2) - (129.5038833927025*np.power(S1,3))/np.power(n,2) - 1168.6259271809045*lm11m1(n,S1) + 1392.0076959003081*lm12m1(n,S1,S2)
    return common + fit
...
def gamma_ps_nf2(n, sx, variation):
    ...
    if variation == 1:
        fit = 82.31518196362009/np.power(-1. + n,2) + 2925.4618402000074/np.power(n,4) - 1096.163535825061/np.power(n,3) + (13.727225301333226*np.power(S1,3))/np.power(n,2)
    elif variation == 2:
        ...
        fit = 446.7679846724957/np.power(-1. + n,2) - 3150.9079125544413/np.power(n,4) - 772.768320207506/(n + np.power(n,2)) + (22.1356222136048*np.power(S1,3))/np.power(n,2)
    elif variation == 5:
        ...
        fit = 766.7194507602298/np.power(-1. + n,2) - 7724.302214687847/np.power(n,4) - 1533.6821976428178/np.power(1. + n,2) + (31.024618755760226*np.power(S1,3))/np.power(n,2)
    elif variation == 7:
        ...
        fit = 234.98236294230156/np.power(-1. + n,2) - 590.6572168807338/np.power(n,2) + (108.18550877168775*S1)/np.power(n,2) + (14.021720692725077*np.power(S1,3))/np.power(n,2)
    elif variation == 11:
        ...
        fit = 327.0839597543234/np.power(-1. + n,2) - 50.67479133179523/(-1. + n) - 1163.4572806201948/np.power(n,4) - 107.44770883692014/np.power(n,3) - 62.85233912390128/np.power(n,2) + 50.67479133179523/n - 231.377853112737/np.power(1. + n,3) + 189.99007406049031/np.power(1. + n,2) - 74.08496392176497/(n*(2. + n)) - 144.11720116681533/(n + np.power(n,2)) - (195.90363960747692*S1)/np.power(n,2) + (15.068424923490317*np.power(S1,2))/np.power(n,2) + (12.085455360923488*np.power(S1,3))/np.power(n,2) - 44.08895905339383*lm11m1(n,S1) + 52.516522934423584*lm12m1(n,S1,S2)
    return common + fit
