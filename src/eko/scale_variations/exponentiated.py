...
#     """
#     if order[1] == 0 :
#         return gamma_variation_pure_qcd(gamma, order, nf, L)
#     else :
#         return gamma_variation_qed(gamma, order, nf, L)
...
def gamma_variation_qed(gamma, order, nf, L):
    ...
    if order[0] >= 4:
        gamma[4, 0] -= (
            3 * beta0 * L * gamma[3, 0]
            + (2 * beta1 * L - 3 * beta0**2 * L**2) * gamma[2, 0]
            + (
                beta.beta_qcd((4, 0), nf) * L
                - 5 / 2 * beta1 * beta0 * L**2
                + beta0**3 * L**3
