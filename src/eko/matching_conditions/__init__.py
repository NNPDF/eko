...
def invert_matching(op_id, ome_members, a_s, method):
    ...
    #         "S.g": a_s ** 2 * np.linalg.inv( ome_members["S_qg"].value ),
    #         "g.S": a_s ** 2 * np.linalg.inv( ome_members["S_gq"].value ),
...
class MatchingCondition(member.OperatorBase):
    ...
    def split_ad_to_evol_map(
        ...
        m = {}
        n = (nf + 1) ** 2 - 1
        if backward_inversion is not None:
            m = invert_matching(op_id, ome_members, a_s, backward_inversion)
        else:
            m = {
